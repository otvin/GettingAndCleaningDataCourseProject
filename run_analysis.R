...
test1 <- require(utils, quietly=TRUE)
test2 <- require(dplyr, quietly=TRUE)

if (test1 == FALSE) {
  stop("Library: utils is not found.  Please install.packages(\"utils\") and try again.")
} else if (test2 == FALSE) {
  stop("Library: dplyr is not found.  Please install.packages(\"dplyr\") and try again.")
} else {
